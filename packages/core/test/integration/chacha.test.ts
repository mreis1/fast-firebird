...
import { connect, createDatabase } from '../../src/index.js';
import { FB_BASE, FB_SERVERS, nextDatabasePath, dropDatabaseAt } from './env.js';
...
describe.each(FB45)('ChaCha wire crypt on Firebird $version', ({ port }) => {
  ...
  it('ChaCha survives blob round-trips', async () => {
    const path = nextDatabasePath(port);
    const seed = await createDatabase({ ...FB_BASE, port, database: path });
    await seed.execute('create table t_cc_blob (id integer, data blob)');
    await seed.disconnect();
    const db = await connect({ ...FB_BASE, port, database: path, wireCryptPlugin: 'ChaCha' });
    try {
      ...
      await db.disconnect();
      await dropDatabaseAt(port, path);
    }
  });
});
